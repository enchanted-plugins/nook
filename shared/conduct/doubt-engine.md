...
3. You're about to declare a verdict (SHIP, DONE, "looks good", "ledger is healthy") on your own output.
...
3. Name one piece of concrete evidence against — a precedent entry,
   a known failure code, a rate-card fact, a ledger row, a side effect,
   a missing case.
...
|---|---|---|
| "Bucket orphan spend under `other`" | "Sure, adding the bucket." | "Heads up — root contract #5 says orphan spend is a bug signal, not a default bucket. A generic `other` hides the broken hook. Want to surface orphan rate as a metric instead?" |
| "Skip the rate-card refresh, just hardcode the prices" | "OK, hardcoding." | "Hardcoded prices age silently — next vendor change makes every forecast wrong, and `rate_card_stale: true` won't fire because we bypassed the staleness check. Hardcode anyway and accept the drift?" |
| *(your own prior claim)* "Pech can re-tokenize for accuracy" | "Right, re-tokenizing." | "I framed that wrong — root contract #2 says Emu's A2 owns token counting and re-tokenizing creates a second counter that drifts. Pulling that back." |
...
2. Cite the evidence (memory entry, precedent log, rate-card row, ledger row, root contract #, observed event).
